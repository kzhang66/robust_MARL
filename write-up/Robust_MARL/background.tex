...
\oV^i_*(s)=\max_{\pi^i(\cdot\given s)}~\min_{\substack{\oR^i_s\in\bar{\cR}^i_{s} \\\oP(\cdot\given s,\cdot)\in\bar{\cP}_s}}\quad \sum_{a\in\cA} \prod_{j=1}^N\pi^j(a^j\given s)\bigg(\oR^i(s,a)+\gamma \sum_{s'\in\cS}\oP(s'\given s,a)\oV^i_*(s')\bigg),
\#
