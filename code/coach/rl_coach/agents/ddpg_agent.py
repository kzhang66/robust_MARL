...
class DDPGAgentParameters(AgentParameters):
    ...
    @property
    def path(self): # this is the key that connects the ``agent_parameters'' and ``agent''
...
class DDPGAgent(ActorCriticAgent):
    ...
    def learn_from_batch(self, batch):
        ...
        # print(batch.next_states(actor_keys))
        # print(actor_keys)

        critic_inputs = copy.copy(batch.next_states(['observation']))


        critic_inputs['action'] = copy.copy(batch.next_states(['action'])) # this was something dealt with "externally", as mentioned core_types.py!!
