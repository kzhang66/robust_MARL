    ...
    def train(self) -> float:
        ...
        if self._should_train():
            ...
            training_schedule = self.call_memory('get_shuffled_training_data_generator', batch_size) if \
                self.ap.is_batch_rl_training else [self.call_memory('sample', batch_size) for _ in range(self.ap.algorithm.num_consecutive_training_steps)]

            # print(training_schedule)
            # print(training_schedule[0])
            # print(training_schedule[0][0])
            tmp_training_schedule = training_schedule[0][0]
            tmp_state = tmp_training_schedule.state
            # print(tmp_state['observation'])
            tmp_obs = np.array([])
            #
            for ii in range(3):
                tmp_obs = np.concatenate((tmp_obs, tmp_state['observation']), axis=0) if tmp_obs.size else tmp_state['observation']

            tmp_training_schedule.state['observation_n'] = tmp_obs
            print(tmp_training_schedule.state)
            # print(type(tmp_training_schedule))
            # print(np.concatenate((tmp_training_schedule.action, tmp_training_schedule.action), axis=0))
            # print(training_schedule[0][0])
