...
class EpisodicExperienceReplay(Memory):
    ...
    def sample_with_index(self, transitions_idx: List[int], episode_idx=-1, is_consecutive_transitions=False) -> List[Transition]:
        """
        ...
        if self.num_complete_episodes() < episode_idx and episode_idx != -1:
            episode_idx = self.num_complete_episodes()-1 # if there are no that many episodes, just take the last one
    ...
    # this is something added to sync the sampling of all agents for given index
    def get_shuffled_training_data_generator_with_index(self, size: int, shuffled_transition_indices: List[int]) -> List[Transition]:
        """
